...
export const Headerside = () => {
  ...
  const headerItems = [
    {name:"introduce", icon: <Homeicon className="stroke-white/60 w-4 h-4 stroke-2 hover:stroke-Primary transition-all ease-in-out duration-300" /> },
    {name:"about", icon: <Abouticon className="stroke-white/60 w-4 h-4 stroke-2 hover:stroke-Primary transition-all ease-in-out duration-300" /> },
  ...
  return (
    <header className="fixed right-5 md:right-6 xl:right-14 top-4 md:top-6 xl:top-14 space-y-12 zindex-999">
      <button
        ...
        className="xl:hidden hm-menu border group/hs border-white/40 transition-all ease-in-out duration-200 hover:border-Primary cursor-pointer rounded-full p-4 w-12 h-12 flex items-center justify-center flex-col space-y-1.5"
      >
      ...
      <div
        className={`fixed top-0  right-0 bg-gradient-to-br from-Eerieblack shadow-side to-gray-800 py-10 md:py-14 px-10 md:px-20 min-w-72 max-w-72 md:max-w-96 md:min-w-96 h-screen transition-all ease-in-out duration-300 ${
        ...
        <nav className="mt-6">
          <ul className="py-6 gap-y-8 flex flex-col  ">
             {headerItems.map((header) => (
              <li key={header.name}>
                <a
                  className="intr text-white flex items-center capitalize text-sm gap-4"
                  href={`#${header.name}`}
                  to="section1"
                  smooth="true"
                  duration={500}
                  data-tooltip-id="my-tooltip"
                  data-tooltip-content={`${header.name}`}
                  place="top"
                >
                 {header.icon}
